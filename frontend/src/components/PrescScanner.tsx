...
export default function PrescScanner({ language, onScanComplete }: PrescScannerProps) {
  ...
  return (
    <div className="w-full max-w-2xl mx-auto bg-white rounded-3xl border-4 border-blue-900 shadow-2xl p-8 my-6">
      ...
      {error && (
        <div className="bg-red-50 border-2 border-red-500 text-red-700 p-4 rounded-xl mb-6 flex items-center gap-3 font-bold text-lg">
      ...
      {stream && !photoData && (
        <div className="relative rounded-2xl overflow-hidden bg-black border-4 border-slate-700 aspect-video mb-6">
          <video ref={videoRef} className="w-full h-full object-cover" playsInline muted />
          <button
            onClick={capturePhoto}
            className="absolute bottom-6 left-1/2 -translate-x-1/2 h-20 px-8 bg-blue-800 hover:bg-blue-900 text-white rounded-full flex items-center gap-3 font-bold text-xl active:scale-95 shadow-2xl border-4 border-white"
          >
          ...
          <button
            onClick={stopCamera}
            className="absolute top-4 right-4 bg-slate-800/80 p-3 rounded-full text-white hover:bg-slate-800"
          >
            <X className="w-6 h-6" />
          </button>
        </div>
      ...
      {photoData && (
        <div className="flex flex-col items-center mb-6">
          <div className="relative rounded-2xl overflow-hidden bg-slate-100 border-4 border-slate-300 max-h-80 w-full flex justify-center mb-6">
          ...
          <div className="flex gap-4 w-full">
            <button
              onClick={startCamera}
              disabled={scanning}
              className="w-1/2 h-20 bg-slate-100 border-4 border-slate-300 hover:bg-slate-200 text-slate-700 font-extrabold text-xl rounded-2xl flex justify-center items-center gap-2 active:scale-95 transition-all"
            >
            ...
            <button
              onClick={submitScan}
              disabled={scanning}
              className="flex-1 h-20 bg-emerald-600 hover:bg-emerald-700 text-white font-extrabold text-2xl rounded-2xl flex justify-center items-center gap-3 active:scale-95 transition-all shadow-lg"
            >
              {scanning ? (
      ...
      {!stream && !photoData && (
        <div className="flex flex-col gap-4">
          <button
            onClick={startCamera}
            className="h-24 bg-blue-800 hover:bg-blue-900 text-white border-4 border-blue-900 rounded-2xl flex justify-center items-center gap-4 active:scale-95 transition-all shadow-lg"
          >
            <Camera className="w-10 h-10" />
            <span className="text-2xl font-black">{t('Use Kiosk Camera', 'कियोस्क कैमरा चालू करें', 'கேமராவை பயன்படுத்தவும்', 'కియోస్క్ కెమెరా ఉపయోగించండి')}</span>
          </button>

          <div className="relative flex py-3 items-center">
            <div className="flex-grow border-t-2 border-slate-300"></div>
            <span className="flex-shrink mx-4 text-slate-400 font-bold text-lg">{t('OR', 'या', 'அல்லது', 'లేదా')}</span>
            <div className="flex-grow border-t-2 border-slate-300"></div>
          </div>

          <label className="h-20 bg-slate-100 hover:bg-slate-200 text-slate-700 border-4 border-slate-300 border-dashed rounded-2xl flex justify-center items-center gap-3 cursor-pointer active:scale-95 transition-all">
            <Upload className="w-8 h-8" />
            <span className="text-xl font-bold">{t('Upload Prescription Image', 'पर्चे की फोटो अपलोड करें', 'கோப்பை பதிவேற்றவும்', 'ప్రిస్క్రిప్షన్ చిత్రాన్ని అప్‌లోడ్ చేయండి')}</span>
